...
/**
 * Validates Nanoid format (21 characters)
 */
...
/**
 * Validates phone numbers (China: 0/86/17951 prefix + 11 digits)
 */
export const isPhone = /^(0|86|17951)?(1\d{10})$/

/**
 * Validates numeric strings (digits only)
 */
export const isNumeric = /^\d+$/

/**
 * Validates Chinese ID card numbers (15 or 18 digits)
 */
export const isIdCard =
  /^(^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$)|(^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[Xx])$)$/

/**
 * Validates Chinese zip codes (6 digits)
 */
export const isZipCode = /^[1-9]\d{5}(?!\d)$/

/**
 * Validates Chinese characters only
 */
export const isChinese = /^[\u4e00-\u9fa5]+$/

/**
 * Regular expression for matching RGBA values
 */
export const matchRgba = /\d+(\d*\.?\d*)/g

/**
 * Regular expression for matching HSLA values
 */
...
/**
 * Validates strings containing both digits and letters
 */
export const hasDigitAndLetter = /^(?=.*\d)(?=.*[a-zA-Z])/

/**
 * Validates strings containing digits, uppercase and lowercase letters
 */
