...
describe('@zid-utils/regexps', () => {
  ...
  describe('validOfMap', () => {
    ...
    it('should validate using mapped validator', () => {
      expect(validOfMap('onlyEmail', 'test@example.com')).toBe(true)
      expect(validOfMap('onlyEmail', 'invalid')).not.toBe(true)
    })
    ...
    it('should validate numeric input', () => {
      expect(validOfMap('onlyNumber', '12345')).toBe(true)
      expect(validOfMap('onlyNumber', 'abc')).not.toBe(true)
    })

    it('should validate phone input', () => {
      expect(validOfMap('onlyPhone', '13812345678')).toBe(true)
      expect(validOfMap('onlyPhone', 'abc')).not.toBe(true)
    })
  })
  ...
  describe('baseValidMap', () => {
    it('should contain all required validators', () => {
      ...
      expect(keys).toContain('onlyUrl')
      expect(keys).toContain('onlyPhone')
    })

    it('should have validators with correct structure', () => {
      baseValidMap.forEach((item) => {
        expect(typeof item.key).toBe('string')
        expect(typeof item.desc).toBe('string')
        expect(typeof item.validator).toBe('function')
      })
    })
  })
})
